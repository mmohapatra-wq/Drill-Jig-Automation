...
# Read-HoleCentersFromModel: read the SELECTED individual HOLES (edge-arc) as the
# layout, the alternative to fasteners. Same result shape as Read-FastenerCentersFromModel
# so the projection/handoff below is identical.
...
function Get-FastenerLayoutRawPoints {
    ...
    try {
        ...
        # Reference the jig holes off the FASTENERS or off the HOLES themselves. Reading
        # HOLES (edge-arc) works on imported assembly geometry where cylinder-surface reads
        # fail, and lets you build the jig from a part whose holes are drilled but has no
        # fastener components. Both reads return the same shape -> identical projection below.
        $srcAns = Read-Host "  Reference off (F)asteners or (H)oles? [F/h]"
        $readHoles = ($srcAns.Trim().ToLower() -eq 'h')
        if ($readHoles) {
            Write-Host ("  HOLE mode: {0}" -f $fName) -ForegroundColor Cyan
            Write-Host "  WARNING: each hole must be its OWN individual feature/entity -- Ctrl-click each" -ForegroundColor Yellow
            Write-Host "  hole SEPARATELY (holes merged into one feature may not resolve to distinct centers)." -ForegroundColor Yellow
            Write-Host "  Then press ENTER." -ForegroundColor Cyan
            Read-Host "  Press ENTER once the holes are selected" | Out-Null
            $read = Read-HoleCentersFromModel -Session $fSess -Model $fModel -TypeObj $fType
        } else {
        ...
        if (-not $read.Ok) {
            ...
            Write-Host "  Run fastener-probe.cmd / holeprobe.cmd to see which read works, or use fastenator.cmd + a file." -ForegroundColor Yellow
            return $null
        }
        $centers = $read.Centers
        $readMethodLbl = $read.ReadMethod
        Write-Host ("  Read {0} {1}." -f $centers.Count, $(if ($readHoles) { 'hole center(s)' } elseif ($fIsAsm) { 'component location(s)' } else { 'bore center(s)' })) -ForegroundColor Green
        # COUNT FEEDBACK: surface the selection accounting so a wrong count is visible.
        if ($readHoles) {
            Write-Host ("  Selection: {0} picked -> {1} distinct hole center(s) (via the '{2}' read)." -f $read.RawSelected, $centers.Count, $read.Which) -ForegroundColor DarkGray
            if ($read.SkippedNoXform -gt 0) { Write-Host ("    - skipped {0} pick(s) with no readable hole (select each hole as its own feature)." -f $read.SkippedNoXform) -ForegroundColor Yellow }
        } elseif ($fIsAsm) {
        ...
        # --holes / assembly: one pick = one hole (reader already deduped) -> NO proximity merge.
        $dt = if ($readHoles -or $fIsAsm) { 0.0 } else { $mg }

        # -Axes = each fastener's own bore axis / the hole read's placeholder axis (parallel
        # to $centers) => project onto the PANEL plane so true hole spacing survives a tilted
        # panel (and a normal-along-a-global-axis panel is not collapsed by a global drop).
