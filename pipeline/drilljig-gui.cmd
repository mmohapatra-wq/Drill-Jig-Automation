...
# Read-HoleCentersFromModel: read the SELECTED individual HOLES (edge-arc) as the layout,
# the alternative to fasteners. Same result shape as Read-FastenerCentersFromModel.
. (Join-Path $ScriptDir 'lib\hole_layout.ps1')
...
function global:Invoke-GuiFastenerLiveRead {
    ...
    # 0. reference the jig holes off the FASTENERS or off the HOLES themselves? Reading
    # HOLES (edge-arc) works on imported assembly geometry where fastener/cylinder reads
    # fail, and lets you build from a part whose holes are drilled but has no fasteners.
    $srcAns = $Wizard.AskInline('Import layout - fasteners or holes?', ("Reference the jig hole layout off:`r`n`r`n  YES = the HOLES themselves -- Ctrl-click each individual hole (each must be its OWN feature/entity). Works when there are no fastener components, or on imported geometry.`r`n`r`n  NO = the FASTENERS -- select the fastener components (assembly) or sweep the part bores.`r`n`r`nClick YES for holes, NO for fasteners."), 'YesNo')
    if ($srcAns -ne 'Yes' -and $srcAns -ne 'No') { return $null }
    $readHoles = ($srcAns -eq 'Yes')
    # 1. confirm the source model is active + the right things are selected
    $selMsg = if ($readHoles) { "SELECT the individual HOLES (Ctrl-click EACH hole separately -- each must be its own feature/entity; holes merged into one feature may not resolve to distinct centers)." } `
              else { "For an ASSEMBLY, SELECT the fastener components first (Ctrl-click them). Select ONE component per hole -- the BOLT SHANKS only, NOT their washers/nuts (a bolt+washer+nut stack reads as 2-3 holes at the same spot)." }
    $ans = $Wizard.AskInline('Import layout - live read', ("Make the SOURCE model (the one with the fasteners/holes) the ACTIVE window in Creo, then click OK.`r`n`r`n$selMsg`r`n`r`nThe jig part is only READ from here -- nothing is modified."), 'OKCancel')
    ...
    try {
        ...
        $read = if ($readHoles) { Read-HoleCentersFromModel -Session $fSess -Model $fModel -TypeObj $fType } `
                else { Read-FastenerCentersFromModel -Session $fSess -Model $fModel -TypeObj $fType -IsAsm $fIsAsm }
        if (-not $read.Ok) {
            $tip = if ($readHoles) { 'Select each hole as its own feature, or run holeprobe.cmd.' } else { 'run fastener-probe.cmd to see which read works on this model.' }
            [void]$Wizard.AskInline('Live read', ("Could not read $(if ($readHoles) {'hole'} else {'fastener'}) centers from '$fName':`r`n`r`n$($read.Message)`r`n`r`nTip: $tip"), 'OK')
            return $null
        }
        $mg = if ($HoleDia -gt 0) { [double]$HoleDia } else { 0.25 }
        # HOLES / ASSEMBLY: NO proximity merge -- one pick = one hole (the reader already
        # deduped). DedupTol=0 so distinct holes/fasteners can NEVER collapse; coincident
        # picks surface via the collision check, not a silent merge.
        $dt = if ($readHoles -or $fIsAsm) { 0.0 } else { $mg }
        ...
        $acct = ''
        if ($readHoles) {
            $acct = "`r`n`r`nSelection: $($read.RawSelected) picked -> $($layout.Count) distinct hole(s)."
            if ($read.SkippedNoXform -gt 0) { $acct += "`r`n  - skipped $($read.SkippedNoXform) pick(s) with no readable hole." }
            $acct += "`r`n`r`nIf that count looks wrong, Ctrl-click EACH hole as its own individual feature and re-read."
        } elseif ($fIsAsm) {
...
$importStep = New-WizardStep -Key 'import' -Title 'Import fastener layout (optional)' -Stage 'Import' -Kind 'choice' -PrimaryText 'Next' `
    ...
    -Build {
        ...
        $btnRead.Add_Click({
            $out = Join-Path $ScriptDir 'artifacts\fastener_layout.json'
...
$layoutStep = New-WizardStep -Key 'layout' -Title 'How are the hole points defined?' -Stage 'Layout' -Kind 'choice' -PrimaryText 'Next' `
    ...
    -Build {
        ...
        if ($c.LayoutMode -eq 'fastener') {
            ...
            $layoutFile = Join-Path $ScriptDir 'artifacts\fastener_layout.json'
