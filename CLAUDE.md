...
**Grid = SEED POINT → drill hole+relief → PATTERN THE HOLE (revised 2026-06-24, the user-asked order).** The grid is now built by patterning the *hole feature*, NOT by patterning datum points. Sequence: **STAGE 2.5** — you create ONE seed datum point at the grid corner (I=0,J=0) and select it; `Resolve-SeedPoint` (ID-only — point id + owning feature id; rejects 0 or >1) sets `$gridPointIDs=@(seed)` + `$orthoPattern=$true` (no seed → `$orthoPattern` false → STAGE 3 falls back to picking pre-existing points). **STAGE 3** drills ONE hole on the seed and captures the new hole feature id(s) (`Resolve-NewFeatureIds` = before/after `Get-FeatureIdSet` diff, both new in this file, bracketing only that fire loop) → `$holeFeatIds`. **STAGE 4** (metal asks / 3DP auto) adds ONE coaxial relief and captures `$reliefFeatIds`. **STAGE 5** (new; gated on `$orthoPattern` AND a captured id) Direction-patterns the hole feature and the relief feature **separately** (same dirs/counts/spacings → each relief stays coaxial with its through-hole) into the Nx·Nz grid via `Build-PointPatternMacro` — which selects its seed by a **FEATURE** tree search, so the same builder patterns a hole feature exactly as it would a datum point (only its header comment changed). Pattern dims (TOP×Nx@CcX, FRONT×Nz@CcZ) come from `$orthoGeo`. **STAGE 5 gate = `VersionStamp` canary ONLY** (deliberately not an exact feature count — a Creo pattern's member/header bookkeeping is build-dependent, so a count check would be a brittle generalization); on a no-change MISS (incl. a null stamp) it prints the manual `Edit > Pattern` steps, and if STAGE 3/4 captured nothing to pattern it says so explicitly (never silent). The point-pattern path (`Invoke-PatternPointGrid`) and the `--auto-grid` offset-table path were **retired** from drilljig's flow (lib functions remain, unused; `--auto-grid` flag removed). Note: `$pid` is a read-only PS automatic — `Resolve-SeedPoint` uses `$ptId` (the holeinator trap). 84 offline cases.

**`Build-PointPatternMacro` widgets RECORDED LIVE (trail.txt.7, 2026-06-24):** mined the existing Creo trail files and found a real Direction-pattern recording. The macro now uses the CONFIRMED tokens — command **`ProCmdGeomPattern`** (the Direction variant; `ProCmdPattern` is the Dimension one), fields `maindashInst0.ui_pat_dir_dir1`/`ui_pat_dir_dir2` (ref collectors), `ui_pat_dir_{1,2}_num_inst` (count), `ui_pat_dir_{1,2}_incr` (spacing), confirm **`dashInst0.stdbtn_1`** (NOT `dashInst0.Done`). The earlier guesses (`ProCmdPatternoptions`, `pattern_type_opt`, `dir1_ref`/`dir1_count`/`dir1_spacing`, `dashInst0.Done`) were ALL wrong. **One residual assumption:** the trail SCREEN-PICKED the direction reference, so the macro feeds the TOP/FRONT datum into each direction collector **by id via `Get-SelectDatumByIdMacro`** (surfenator's proven up-to-plane Datum tree-search feed) — that a pattern direction collector accepts this identical feed is the one unproven step; canary + manual fallback cover it. (`Box … | hole dia … | depth …` readout is ASCII — a literal `Ø` mojibake'd under PS 5.1.)
