...
### csysinator.cmd
Creates a datum **COORDINATE SYSTEM at a ROUND edge's center** — the "change of coordinate system using a round edge" flow. You select **ONE round edge** (a hole/fastener rim — the point is that it's ROUND so its **center** can be read); that circle center becomes the CSYS **ORIGIN**. You then orient **+X toward a second reference** — a datum **PLANE** that points in X (per the user, "pick plane going in x direction"), or another edge. Standalone; open the jig **PART** (not `.asm`). Dot-sources `lib\creo_geometry.ps1`.

**Why a round edge (this REPLACES the old 2-hole-axis csysinator):** a datum CSYS needs a **point-like ORIGIN** reference. Two parallel bore **axes** give only a LINE, which UNDER-defines the origin → the dialog stalls demanding more (the exact wall the old 2-axis csysinator hit — see [[project_index_frame]]). A round edge's **center is a well-defined point**, which is precisely what the origin collector wants, so the round-edge route fixes the origin problem the axis route never solved.

**Center read = descriptor read, NEVER `IpfcPoint.Point`.** A solid circular edge is an **ARC** (docs: solid edges are LINE/ARC/SPLINE/BSPLINE; CIRCLE is "reserved"). `$edge.GetCurveDescriptor()` → `IpfcArcDescriptor`/`IpfcCircleDescriptor` exposing **`.Center`** (IpfcPoint3D) + **`.Radius`** — read via `Get-EdgeArcCenter` (new, in `lib\creo_geometry.ps1`, sibling to `Get-CylinderAxisFromSurface`) using the SAME proven descriptor-read family as the live cylinder `.Origin` read, NOT the crash-prone `IpfcPoint.Point`. This is **best-effort / live-unverified** (`arc-descriptor-center-radius` = docs-only): if the center can't be read on a build, the tool STILL creates the csys and just verifies visually — the center is a verification anchor, not a gate.

**Create = replay the operator's RECORDED MAPKEY (2026-07-06), split around TWO mandatory picks.** The mapkey settled the design: csys references are MOUSE-PICKED (two `@PAUSE_FOR_SCREEN_PICK`) — the by-ID auto-orient was WRONG and is removed. Flow: **(A)** `Get-CsysOpenMacro` opens `ProCmdDatumCsys`, primes `t1.OriginPlacement`, toggles `pg_vis_tab tab_2→tab_1` → **PICK 1: the round hole edge** (origin = its center, AND supplies direction 1). **(B)** `Get-CsysDir1Macro` switches to `tab_2`, assigns that direction **`t2.AxisMenu1` = `Axis_Y`** + `t2.FlipBtn1` (so +Y aligns to original Y), primes `t2.DirectionTable2` → **PICK 2: the plane for +X** (e.g. TOP). **(C)** `Get-CsysFinishMacro` fires `t2.FlipBtn2` + refresh, then **OK `stdbtn_1`** (`-WithOK`; the recording stopped before OK, the tool adds it). Only **`Axis_Y`** is assigned (to the origin's direction); **X is IMPLICIT** (the 2nd direction takes the remaining axis) — there is **NO `AxisMenu2`/`Axis_X`** (my earlier inference, refuted by the recording). All widgets recorded-live in the mapkey (fact `datumcsys-dialog-sequence`, `verifiedOn 2026-07-06`).
- **You make exactly TWO picks; the tool drives tabs/axis/flips/OK.** Not hands-free (a `RunMacro` can't replay a pick), but everything else is scripted. **FLIPS are geometry-specific** (FlipBtn1/2 were right for the operator's hole/plane orientation; a differently-drilled hole may come out with an axis reversed) — the post-create report says so and asks for a visual check. `--probe` runs everything EXCEPT the final OK so the csys preview can be eyeballed first.

**Verify = feature-diff canary (the GATE) + post-OK read-back (advisory).** `Count-NewFeatures == 1` is the gate ("created" = a real feature appeared, never "macro fired" — [[feedback_canary_must_not_assume_on_failure]]). **ALL COM reads happen only AFTER OK (dialog closed)** — `Read-NewCsysTransform` diffs `ListItems(ITEM_COORD_SYS)` → new `IpfcCoordSystem` → `Read-CoordSysTransform` (`.CoordSys` → `IpfcTransform3D` `.GetOrigin()/.GetXAxis()/...`, `coordsystem-coordsys-transform-read` = docs-only), reports origin/+X/+Y; axes verified VISUALLY (plane normals read null). **FREEZE FIX (live 2026-07-07, `com-read-deadlocks-while-dialog-open`):** an earlier build read the picked origin edge's center from the selection buffer MID-DIALOG — a COM data read while the csys dialog is open DEADLOCKS (Creo's main thread is in the dialog pick loop; only `RunMacro` UI events are safe mid-dialog). That read was removed; a post-OK ENTER-gate confirms the dialog is closed before any verify read. Flags: `-v` verbose, `--probe` (all but OK). Offline: `lib\tests\run_csys_tests.ps1` (47 cases — descriptor reads incl. arc-no-center degrade + throwing-axis, AND the three csys macro segments' token structure AST-extracted from the .cmd). **NOT yet run live end-to-end** in this mapkey-replay form; open items: whether OK lands after the 3-segment split (ODUI dialog persisting across the 2 pauses) and whether the recorded flips suit the operator's geometry.
...
datum points → corner round → drill → chip-relief SLOTS) with **no console typing**. Additive: drilljig.cmd
...
proven Creo engine, called VERBATIM) + the EXISTING `lib\orthogrid_gui.ps1` editors.
