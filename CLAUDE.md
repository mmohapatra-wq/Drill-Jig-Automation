...
4. Bushing pick (`Invoke-BushingPick`) — **ID → STANDARDIZED length menu → OD tie-break** (user 2026-07-21, technician rule: sleeve lengths are standardized to the bore, so a 1/2" ID is normally 1/2" long, a 3/4" ID 3/4" long). **ID** first (the bore; its resolved-hole preview — `-> hole 0.5"` or `N OD options` — now rides the ID card via `Get-IdOdOptions`). Then a **FIXED length menu {1/2, 3/4, 1} + Custom** (`Get-BushingLengthOptions`): the length matching the ID is **RECOMMENDED** (console: blank ENTER accepts it; GUI: the card is marked), and **Custom** takes any typed value (decimal / `3/8` / mixed `1 3/8`, validated by `Resolve-BushingLengthInput`, rejects ≤0). Because the fixed menu is **decoupled from the catalog's length rows**, **OD is re-keyed on ID ALONE** (the union of the ID's distinct ODs) — auto-resolved when unique, offered as a tie-breaker when the ID reaches >1 OD (OD IS the drilled hole, never silently chosen; the tie-break no longer references length). `Resolve-BushingPickRow` returns the real SKU row when an exact (ID, OD, length) exists, else a **synthesized** pick (`PartNumber = "(custom length)"`) carrying the chosen length. **CRITICAL: `.Length` is always the chosen (possibly custom) length** — it flows to `BushingLength` → drilljig STAGE 2 SIDE-plane offset (= plate thickness); `.OD` is always a real catalog value. These four pure helpers live in `lib\drilljig_core.ps1` (shared by drilljig.cmd + drilljig-gui.cmd) and are **mirrored as local copies in jiginator.cmd / jiginator.ps1** (self-contained). Machinist fractions ("3/4", "1 3/8") are shown from the row `EasyName` via `Get-FracLabel`.
...
- **Offline:** `lib\tests\run_fastener_tests.ps1` (70 cases incl. `Set-LayoutMargin` re-anchor + the edge-margin bug/fix) and `lib\tests\run_tests.ps1` (the shared `Read-FastenerCentersFromModel` assembly branch: dedup-by-component-id + transform-origin read + never-throws, stubbed COM). `fastener_layout.json` + `fastener_probe_report.txt` are gitignored.
