...
## Repo Layout
The `.cmd` tools are grouped by role into subfolders (shared code stays at root):
- **`pipeline/`** — end-to-end drill-jig flow: `drilljig`, `drilljig-gui`, `drilljig3d`, `drilljig3d-gui`, `jiginator` (+ `jiginator.ps1`)
- **`tools/`** — standalone geometry tools: `boxinator`, `holeinator`, `csysinator`, `cornerinator`, `slotinator`, `pocketinator`, `fastenator`, `holelayoutinator`, `nodelator`, `flipenator`, `radinator`, `surfenator`, `thickenator`, `gauginator`, `diminator`, `gripenator`
- **`previews/`** — no-Creo visualizers: `bushing-preview`, `bushing-3d-preview`, `drilljig-3d-preview`, `drilljig-3d-webview`
- **`probes/`** — read-only diagnostics + trail recorders: `fastener-probe`, `holeprobe`, `drilljig3d-probe`, `csys-negoffset-probe`, `slotplane-probe`, `tangent-plane-probe`, `slotpat-probe`, `holepat-probe`, `csystrf-probe`
- **`lib/`** (+ `lib/tests/`), **`data/`**, **`docs/`** — unchanged, at the repo root.

Inline `NAME.cmd` references throughout this doc are left **unqualified** for brevity — each lives under its role folder above. Test harnesses locate a tool via `Resolve-RepoFile` (searches root + the four subfolders).

## Script Conventions
- All scripts use the same CMD/PowerShell hybrid header (lines 1-6). **Line 4 re-anchors `$ScriptDir` to the repo ROOT** — `$ScriptDir=((Split-Path -Parent ('%~dp0'.TrimEnd('\')))+'\')` — so a tool one level down under `pipeline/`/`tools/`/`previews/`/`probes/` still resolves every `Join-Path $ScriptDir 'lib\…'`/`'data'`/`'docs\…'` and the cross-tool handoff files (`last_jig_spec.json`, `fastener_layout.json`, `hole_layout.json`) to the root. `$ScriptDir` therefore == the repo root (its value before the reorg), so nothing downstream changed.
...
- **`lib\drilljig_core.ps1`** — the SHARED drill-jig engine: the proven helper bodies behind
  ...
  `Wait-ModelModified`, `Get-BodyList`, and the HANDS-FREE slot-pattern seed re-select
  (`Select-FeatureById` — raw-COM `CreateModelItemSelection`+`AddSelection` of a FEATURE, the
  edginator channel; `Invoke-SlotPatternFromSeed` — select the seed + fire one `ProCmdGeomPattern`,
  canary-gated, `@{Selected;Changed;Reason}`; FIX 4). The COM helpers read a one-time scope set by
...
4. Bushing pick (`Invoke-BushingPick`) — TWO shapes, chosen by the catalog spec's filter column:
   - **METAL removable-bushing path (OD-FIRST — user 2026-07-22; PFD MOVED TO ID-FIRST SLEEVE 2026-07-23).** **METAL → PFD no longer uses this OD-first path** — its tree outcome label was changed from `3/4 OD removable bushings` to `3/4 ID sleeves`, so it now takes the **ID-FIRST sleeve** path (see the 3D-print bullet below): the buttons show the **ID** (only `3/4`), the 2D/3D schematic renders **HEADLESS** (sleeve EasyName → `Get-BushingHeadDia`=0), and the custom-hole-OD + recommended-length machinery is inherited unchanged. **This was a single decision-tree-JSON outcome-label edit** — `Get-CatalogSpec` routes the `3/4 ID sleeves` label to `bushings.csv` on an ID filter, `Test-OdFirstSpec` returns false, and every downstream consumer (console `drilljig.cmd`/`jiginator.cmd`/`.ps1` + `drilljig-gui.cmd`) flips with no code change. Only **METAL → Hand Drill** remains OD-first: the tree resolves it to a **removable bushing filtered by OD** (`3/4 OD` + `1/2 OD`), and the drilled jig hole **IS** the removable bushing's OD — the technician doesn't care about the bore ID. So the OD-first (Hand Drill) path **DISPLAYS THE OD (never the ID), SKIPS the ID question entirely, and only `0.5"`/`0.75"` ODs ever show**, then asks the standardized length. Discriminator: `Test-OdFirstSpec` = the spec carries an **OD-column** filter (`Get-CatalogSpec` emits an OD filter ONLY for the metal removable leaves; the 3D-print sleeve leaves filter on **ID**) — no label re-parsing. `Get-OdGroups` gives the distinct OD cards (`-> hole 0.5"`/`0.75"`); the length menu is the SAME fixed `{1/2, 3/4, 1} + Custom`, but **recommended from the OD value** (`0.5 OD → 1/2"`, `0.75 OD → 3/4"`) via `Get-BushingLengthOptions -Id <OD>`. `Resolve-OdBushingPick` returns the pick with **`ID = '(any)'`**, `OD` = the drilled hole, `Length` = the chosen value (borrows a real `PartNumber` if a SKU at that (OD, length) exists, else `(ID unspecified)`). There is NO OD tie-break (OD is already chosen). The three pure helpers `Test-OdFirstSpec`/`Get-OdGroups`/`Resolve-OdBushingPick` live in `lib\drilljig_core.ps1` (shared) and are mirrored into `jiginator.cmd`/`.ps1`. In `drilljig-gui.cmd` the metal spec sets `$ctx.BushOdFirst=$true` + a new `'od1'` bush sub-stage (OD cards); `Set-BushLengthPick`'s OD-first branch resolves + finishes.
...
**CHIP-RELIEF SLOTS in the GUI (2026-07-07; CONFIRMED LIVE 2026-07-08).** The old relief-hole block (inside the drill RUN step) and the entire relief-PATHS wizard step were REMOVED; a new **`Relief` stage** = two steps mirroring box-a/box-b: **slot-a** (create the slot-edge guide planes via `New-SlotGuidePlanes`, then arm the seed sketch on the SIDE face; material gate — 3DP auto / metal asks via `Show-WizardMessage`; skips if no `$c.OrthoGeo` layout) and **slot-b** (finish the seed cut via `Build-CutFinishMacro`, verify direction with a Yes/No — wrong → `ProCmdEditUndo` + flip + re-arm the sketcher + stay so the operator redraws; right → the seed's feature id is captured (`Get-FeatureIdSet` diff around the confirmed cut → `$c.SeedFeatId`) and each pattern group AUTO-RESELECTS that seed via `Invoke-SlotPatternFromSeed` (FIX 4, raw-COM `Select-FeatureById`) + `Build-SlotPatternMacro` — **hands-free, no per-pattern tree click**; a canary miss falls back to the old AskInline "select the seed" prompt for that pattern). Reuses ONLY stored context (`$c.HoleDiaFinal`, live SIDE offset, `$c.OrthoGeo`, `$c.Made/$c.SidePlane`, `$c.BodyIndex`, `$c.SeedFeatId`) — no re-entry. Honesty chip `slots: built | unverified | aborted | skipped`.
...
- **FIX 4 (HANDS-FREE seed re-select, user 2026-07-23 "you dont need to ask the user, you can manually just reselect that feature and do the patterns until all of the rows have the slots") — auto-reselect the seed, no manual tree click.** FIX 1 found the *tree-SEARCH* dialog channel (`Get-SelectByIdMacro`) does NOT register as a pattern target — so the operator had to click the seed. But edginator (2026-06-24, `lib\edge_round.ps1`) proved a **DIFFERENT** selection channel — RAW COM `CMpfcSelect.CreateModelItemSelection(item,$null)` + `CurrentSelectionBuffer().AddSelection` — DOES feed a dashboard command (`ProCmdRound`) on the SAME imported/foreign jig bodies where the tree search is dead, and it was **never tried for the pattern**. FIX 4 uses it: the seed slot's feature id is captured at the cut (console: `Invoke-VerifiedSeedCut` returns `.FeatId`; GUI: a `Get-FeatureIdSet` diff around the confirmed cut → `$c.SeedFeatId`), then `Invoke-SlotPatternFromSeed` (`lib\drilljig_core.ps1`) re-selects it via the new pure-COM `Select-FeatureById` (feature-typed) and fires `Build-SlotPatternMacro` — **NO manual click**, once per pattern group (the GUI's regular + accommodation patterns loop hands-free). **Canary-gated**: it returns `@{ Selected; Changed; Reason }`, and a caller treats `(Selected -and Changed)` as success; on ANY miss (id unresolved / COM select didn't land / pattern didn't change the model) it **FALLS BACK to the old manual reselect prompt** — so a build where the raw-COM select doesn't register as the pattern target is never stuck (never regresses FIX 1's proven manual path). Wired at all three sites: `slotinator.cmd`, `drilljig.cmd` STAGE 4, `drilljig-gui.cmd` slot-b. Offline: `run_wizard_tests.ps1` (16 gating cases: guards for seed-id≤0 / no-dir-datum / select-fail don't fire the macro; success fires exactly one `ProCmdGeomPattern` feeding the dir datum by id; a no-change canary miss reports `Changed=$false`). **LIVE-UNVERIFIED (behavioral): whether `ProCmdGeomPattern` HONORS a raw-COM-`AddSelection`'d feature as its target is the load-bearing unknown** — it's the exact analog of edginator's proven `ProCmdRound` case, but not yet confirmed live for the pattern. If it doesn't register live, the manual fallback fires (functional, just not hands-free); confirm live and, once seen, add a `slot-pattern-seed-raw-com-select` fact.
...
### curvedhole-probe.cmd
**MUTATING probe (creates ONE throwaway datum point + optionally ONE hole; confirm-gated + canary-gated) — settles the AFTER-surface curved-jig workflow the operator recorded (`docs\curved_hole_from_fastener_planes.mapkey.txt`, mapkey `curvedholes`, 2026-07-27).** In the TOP-DOWN ASSEMBLY (curved-surface part + fasteners part + drilljig part), each fastener part instance carries its own TOP/SIDE/FRONT datum planes at the fastener; the workflow is: select that fastener's 3 planes → `ProCmdDatumPointGeneral` + `stdbtn_1` → a datum POINT at their INTERSECTION (= the fastener location; SAME command proven for the flat 3-plane point, `point-at-3-plane-intersection-by-id`) → `ProCmdHole` placed BY REFERENCE (`hole_fb_plcmnt_page.3.0` — `PH.ui_hole_prim_ref_cui_lst` = the point/placement, `PH.ui_hole_ft_dir_cui_lst` = a PICKED drilljig reference that sets the drill ORIENTATION) drilled THROUGH the drilljig part. **Every widget token is transcribed VERBATIM from the recording (mine-don't-guess — [[reference_mine_trail_files_for_widgets]]); the direction reference is an `@PAUSE_FOR_SCREEN_PICK`, so the hole recipe SPLITS around the operator's pick (a RunMacro cannot screen-pick).** **ASSEMBLY REQUIRED** (the fastener component planes only exist in the assembly — the INVERSE of the usual `.prt`-only guard). Flow: `[1]` operator Ctrl-clicks the fastener's 3 planes → ID-ONLY diagnostic dump (`Id`/`Type`/`GetName`/`Path.ComponentIds`/`SelectionString` per item; classifies TOP/SIDE/FRONT via `Resolve-PlaneRole`; reads the fastener component origin via the PROVEN `Path.GetTransform($true).GetOrigin()` so the report names where the point SHOULD land — the point's own coords are NEVER read, `IpfcPoint.Point` crashes); y/N confirm; `[2]` fire the point macro → `ITEM_POINT` diff + `VersionStamp` canary (a NEW point must appear, else a MISS — [[feedback_canary_must_not_assume_on_failure]]); `[3]` (unless `--no-hole`) fire hole part A → operator picks the direction ref → hole part B → feature-diff canary. Flags: `--no-hole` (point only), `-v`. Dot-sources `lib\creo_geometry.ps1` + `lib\drilljig_core.ps1` (`Initialize-DrilljigCore`/`Get-FeatureIdSet`/`Wait-ModelModified`). Writes `curvedhole_probe_report.txt` (gitignored). Fact `hole-by-reference-and-direction` (docs-only until this probe confirms it live). Design note (user): the chip-relief SLOT plane can be the fastener's own TOP plane (no tangent plane needed for that half). NEXT once confirmed live: automate the 3-plane selection from the KNOWN fastener component (`Path`/`ComponentIds`) via a path-qualified raw-COM selection — NOT the fragile `PHTLeft.AssyTree` tree-node keys (`T3 6 0`) the recording used. Mirrors `tangent-plane-probe`/`fastener-probe`.
